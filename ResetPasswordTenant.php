...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(isset($_POST["email"]) && !empty($_POST["email"]) && isset($_POST["password"]) && !empty($_POST["password"])
       && isset($_POST["newPassword"]) && !empty($_POST["newPassword"]) && isset($_POST["confirmNewPassword"]) && !empty($_POST["confirmNewPassword"])){
        ...
        if($_POST["newPassword"] === $_POST["confirmNewPassword"]){
            ...
            // check inputed name with username in our table: Tenant
            $stmt1=$con->prepare("SELECT Email,Password FROM tenant WHERE Email=?");
            ...
            {
                    // it belongs to table: tenant

                    $stmt1->close();
                    // then update the password in table: tenant
                    $stmt2 = $con->prepare("UPDATE tenant SET Password = ? WHERE Email = ?");
                    if (!$stmt2) {
            ...
            // not match any information in two tables
                 ...
                 header("Location: ResetPasswordTenant.html?error=incorrect_emailorpassword");
                 exit(); 
            }
        }
        else{
         ...
         // then stay the same page let user re-input
         // then stay the same page let user re-input
         header("Location: ResetPasswordTenant.html?error=passwords_dismatch");
         exit(); 
        }
    ...
    // if one of them is empty
         ...
         // then stay the same page let user re-input
          // then stay the same page let user re-input
         header("Location: ResetPasswordTenant.html?error=fields_empty");
         exit(); 
    }
}
